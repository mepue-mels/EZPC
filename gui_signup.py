...
def main():
    ...
    def create_account():
        if len(userbox.get()) < 3 or len(passbox.get()) < 3:
            print("Try again!")
            userbox.delete(0, END)
            passbox.delete(0, END)
            messagebox.showerror(title="Try again!", message="Username or password too short.")
        else:
            user_found = create_user(userbox.get(),passbox.get())
            if user_found:
                print("Wait a minute!")
                userbox.delete(0, END)
                passbox.delete(0, END)
                messagebox.showerror(title="Wait a minute!", message="Username already exists in the database. Try entering a new one.")
            else:
                messagebox.showinfo(title="Nice!", message="User created successfully.")
                open_login()
    ...
    text1 = Label(frame, text = '(at least 3 characters)', fg = color_black, bg = color_lightgray, font = ("Ubuntu Mono", 8))
    ...
    text2 = Label(frame, text = '(at least 3 characters)', fg = color_black, bg = color_lightgray, font = ("Ubuntu Mono", 8))
    text2.place(x=0,y=145)
    ...
    Button(frame, image = confirm, bg = color_lightgray, borderwidth = 0, highlightthickness = 0, relief = "flat", command = create_account).place(x=200,y=175+60)
