...
def verify():
    # input controller for user handling here
    user_found = True # temp login
    if user_found:
        open_ezpc()
    else:
...
Label(frame, text = "Don't have an account yet?", font = ("Ubuntu Mono", 8), fg = color_black, bg = color_lightgray).place(x=0,y=138) 
Button(frame, text = "Create one.", font = ("Ubuntu Mono", 8), fg = color_blue, bg = color_lightgray, borderwidth = 0, highlightthickness = 0, relief = "flat", command = open_signup).place(x=165,y=139)   
Button(frame, image = enter, bg = color_lightgray, borderwidth = 0, highlightthickness = 0, relief = "flat", command = open_ezpc).place(x=200,y=175)
